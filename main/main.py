...
# Use genfromtxt for read data_train from file .csv
...
ma = np.dot(Xbar.T, Xbar) + np.dot(7, I)
# lamda = 29999
...
# my_data = genfromtxt('./data_train/20142090-test.csv', delimiter=',')
# X1 = my_data[:,:-1]
# one = np.ones((X1.shape[0], 1))
# X1 = np.concatenate((one, X1), axis = 1)
# y1 = np.dot(w[0][0], X1[0])
